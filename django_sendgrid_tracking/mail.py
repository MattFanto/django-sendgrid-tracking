...
def create_send_email(sender, message, fail_flag, **kwargs):
    ...
    create_sent_mail_object = getattr(message, 'create_sent_mail_object', None)
    if callable(create_sent_mail_object):
        message.create_sent_mail_object(x_message_id=x_message_id, fail_flag=fail_flag)
    else:
